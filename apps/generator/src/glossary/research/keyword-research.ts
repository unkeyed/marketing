...
export async function keywordResearchStep({
  term,
  ...
  return withRetry(
    async () => {
      ...
      if (!searchQuery) {
        throw new Error("Unable to generate search query");
      }
      ...
      await db
        ...
        .onConflictDoUpdate({
          target: keywords.inputTermAndKeywordHash,
          set: {
            updatedAt: sql`NOW()`,
          },
        });
    ...
    { maxAttempts: 3, label: "keywordResearch" },
  );
}
