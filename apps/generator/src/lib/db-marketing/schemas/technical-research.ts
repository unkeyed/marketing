...
export const domainCategoryEnum = pgEnum(
  "domain_category",
  domainCategories.map((c) => c.name) as [DomainCategory, ...DomainCategory[]],
);

export const technicalResearch = pgTable(
  "technical_researches",
  {
    id: serial("id").primaryKey(),
    inputTerm: varchar("input_term", { length: 767 }).notNull(),
    domainCategory: domainCategoryEnum("domain_category").notNull(),
    hashedExaSearchResponseWithoutContent: varchar("hashed_exa_search_response_without_content", {
      length: 64,
    }).notNull(), // SHA-256 hash
    exaSearchResponseWithoutContent: jsonb("exa_search_response_without_content")
      .$type<SearchResponse<{ [k: string]: never }>>()
      .notNull(),
    exaSearchResponseWithContent: jsonb("exa_search_response_with_content")
      .$type<SearchResponse<{ summary: true; text: true }> | null>()
      .default(null),
    searchEvaluation: jsonb("search_evaluation").$type<SearchEvaluation>().default(null),
    exaScrapedContent: jsonb("exa_scraped_content")
      .$type<SearchResponse<{
        summary: {
          query: string;
        };
  ...
  (table) => [
    index("technical_researches_input_term_idx").on(table.inputTerm),
...
export const exaScrapedResults = pgTable(
  "exa_scraped_results",
  {
    id: serial("id").primaryKey(),
    inputTerm: varchar("input_term", { length: 767 }).notNull(),
