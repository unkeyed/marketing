...
import { type BlogOutlineSection, blogPosts } from "@/lib/db-marketing/schemas/blog-posts";
  ...
  return withRetry(
    async () => {
      ...
      const audienceGuidance = {
        beginner:
          "Write for developers new to these topics. Define all technical terms on first use. Use analogies. Include beginner-friendly code examples with comments explaining each line.",
        intermediate:
          "Write for developers with working knowledge. Skip basic definitions but explain nuanced details. Include practical code examples that demonstrate real-world usage patterns.",
        advanced:
          "Write for senior developers. Focus on edge cases, performance implications, and architectural decisions. Include advanced code examples showing optimizations and trade-offs.",
      }[audienceLevel];
...
- The output must be ready to publish as-is with zero editing`,
      ...
      await db.update(blogPosts).set({ content: finalContent }).where(eq(blogPosts.id, blogPostId));
