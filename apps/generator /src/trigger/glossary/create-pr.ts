...
export const createPrTask = task({
  ...
  run: async ({
    ...
    console.info(`🟢 [createPrTask][owner:${owner}][repo:${repo}][term:${input}] Start PR creation`);
    ...
    if (existing?.githubPrUrl && onCacheHit === "stale") {
      console.info(`✅ [createPrTask][owner:${owner}][repo:${repo}][term:${input}] Cache hit, returning PR: ${existing.githubPrUrl}`);
      return { entry: { id: existing.id, inputTerm: existing.inputTerm, githubPrUrl: existing.githubPrUrl } };
    }

    // 2. Prepare MDX file content
    console.info(`📄 [createPrTask][owner:${owner}][repo:${repo}][term:${input}] Preparing MDX file`);
    const entry = await db.query.entries.findFirst({
      where: eq(entries.inputTerm, input),
      orderBy: (entries, { asc }) => [asc(entries.createdAt)],
    });
    if (!entry?.dynamicSectionsContent) {
      console.error(`❌ [createPrTask][owner:${owner}][repo:${repo}][term:${input}] No dynamicSectionsContent found`);
    ...
    if (!entry.takeaways) {
      console.error(`❌ [createPrTask][owner:${owner}][repo:${repo}][term:${input}] No takeaways found`);
      throw new AbortTaskRunError(
    ...
    console.info(`🔍 [createPrTask][owner:${owner}][repo:${repo}][term:${input}] Check file in main branch`);
    ...
    if (fileExistsInMain && fileIsIdenticalInMain) {
      console.info(`🟢 [createPrTask][owner:${owner}][repo:${repo}][term:${input}] File identical in main, early return`);
      return { entry: { id: entry.id, inputTerm: entry.inputTerm, githubPrUrl: entry.githubPrUrl } };
    } else if (fileExistsInMain) {
      console.info(`🟡 [createPrTask][owner:${owner}][repo:${repo}][term:${input}] File exists in main, content differs`);
    } else {
      console.info(`🟠 [createPrTask][owner:${owner}][repo:${repo}][term:${input}] File missing in main`);
    }

    // 4. List all branches and filter for relevant ones (reuse branches)
    console.info(`🌿 [createPrTask][owner:${owner}][repo:${repo}][term:${input}] List all branches`);
    const branchListResult = await tryCatch(
      octokit.repos.listBranches({ owner, repo })
    );
    if (branchListResult.error) {
      console.error(`❌ [createPrTask][owner:${owner}][repo:${repo}][term:${input}] Failed to list branches: ${branchListResult.error}`);
    ...
    const relevantBranches = allBranches
      ...
      .map((b: any) => b.name);
    console.info(`🌿 [createPrTask][owner:${owner}][repo:${repo}][term:${input}] Found ${relevantBranches.length} branches including slug '${slug}'`);
    ...
    if (relevantBranches.length > 0) {
      ...
      console.info(`🔄 [createPrTask][owner:${owner}][repo:${repo}][branch:${branch}] Start branch check`);
      ...
      // Branch summary log
      console.info(`🔄 [createPrTask][owner:${owner}][repo:${repo}][branch:${branch}] file:${branchFileExists ? (branchFileIsIdentical ? 'unchanged' : 'changed') : 'missing'} pr:${prExists ? 'open' : 'none'}`);
      console.info(`⚡ [createPrTask][owner:${owner}][repo:${repo}][branch:${branch}] Case: ${caseKey}`);
      switch (caseKey) {
        case "branch-existant:fileDiff-unchanged:pr-existant":
          // Case 1: File exists, is identical, and PR exists
          console.info(`✅ [createPrTask][owner:${owner}][repo:${repo}][branch:${branch}] File unchanged, PR exists: ${openPr?.html_url}`);
        ...
        case "branch-existant:fileDiff-unchanged:pr-inexistant":
          ...
          console.info(`⚡ [createPrTask][owner:${owner}][repo:${repo}][branch:${branch}] File unchanged, creating PR`);
          ...
          if (!newPr1Result.data) {
            console.error(`❌ [createPrTask][owner:${owner}][repo:${repo}][branch:${branch}] Failed to create PR`);
        ...
        case "branch-existant:fileDiff-changed:pr-existant":
          ...
          console.info(`⚡ [createPrTask][owner:${owner}][repo:${repo}][branch:${branch}] File changed, updating file in PR`);
          ...
          if (!updateResult1.data) {
            console.error(`❌ [createPrTask][owner:${owner}][repo:${repo}][branch:${branch}] Failed to update file`);
        ...
        case "branch-existant:fileDiff-changed:pr-inexistant":
          ...
          console.info(`⚡ [createPrTask][owner:${owner}][repo:${repo}][branch:${branch}] File changed, updating file and creating PR`);
    ...
    console.info(`🆕 [createPrTask][owner:${owner}][repo:${repo}][term:${input}] No branch, creating new branch/PR`);
    const timestamp = Date.now();
    const newBranchName = `${branchPrefix}_${timestamp}`;
    // Get the SHA of the latest commit on the base branch
    console.info(`🔍 [createPrTask][owner:${owner}][repo:${repo}][branch:${newBranchName}] Get SHA for base branch`);
    const refResult = await tryCatch(
      octokit.git.getRef({ owner, repo, ref: `heads/${baseBranch}` })
    );
    if (!refResult.data) {
      console.error(`❌ [createPrTask][owner:${owner}][repo:${repo}][branch:${newBranchName}] Failed to get ref for base branch`);
      throw new AbortTaskRunError(`Failed to get ref for base branch ${baseBranch}`);
    }
    // Create new branch
    console.info(`🌿 [createPrTask][owner:${owner}][repo:${repo}][branch:${newBranchName}] Creating branch`);
    ...
    if (!createBranchResult.data) {
      console.error(`❌ [createPrTask][owner:${owner}][repo:${repo}][branch:${newBranchName}] Failed to create branch`);
    ...
    console.info(`📄 [createPrTask][owner:${owner}][repo:${repo}][branch:${newBranchName}] Creating file`);
    ...
    if (!createFileResult.data) {
      console.error(`❌ [createPrTask][owner:${owner}][repo:${repo}][branch:${newBranchName}] Failed to create file`);
      throw new AbortTaskRunError(`Failed to create file ${filePath} in branch ${newBranchName}`);
    }
    // Create PR
    console.info(`⚡ [createPrTask][owner:${owner}][repo:${repo}][branch:${newBranchName}] Creating PR`);
    ...
    if (!createPrResult.data) {
      console.error(`❌ [createPrTask][owner:${owner}][repo:${repo}][branch:${newBranchName}] Failed to create PR`);
    ...
    console.info(`✅ [createPrTask][owner:${owner}][repo:${repo}][term:${input}] Done, PR: ${createPrResult.data.data.html_url}`);
