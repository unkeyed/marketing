...
import Exa, { type ContentsOptions, RegularSearchOptions, SearchResponse, SearchResult } from "exa-js";
import { DomainCategory, technicalResearch } from "./db-marketing/schemas";
import { CacheStrategy } from "@/trigger/glossary/_generate-glossary-entry";
...
export const composeSearchOptionsWithoutScraping = (props: { numResults: number, domain: string }) => {
...
export async function getOrCreateSummary({
    ...
    const queryExistingTechnicalResearches = await tryCatch(db.query.technicalResearch.findMany({
    ...
    if (queryExistingTechnicalResearches.error) {
      console.warn(`Error querying entry for term ${connectTo.term}:`, queryExistingTechnicalResearches.error);
      throw new AbortTaskRunError(`Error querying entry for term ${connectTo.term}: ${queryExistingTechnicalResearches.error}`);
    };
    if (!queryExistingTechnicalResearches.data?.some((r) => r.exaScrapedContent)) {
      throw new AbortTaskRunError(`No technical research found for term ${connectTo.term}. Run technical research first.`);
    }
    const existingTechnicalResearch = queryExistingTechnicalResearches.data.find((r) => r.exaScrapedContent?.results.find((r) => r.url === url));
  ...
  **Instructions:**
  ...
  Summarize the following content for the term "${connectTo.term}".`;
    ...
    const updatedTechnicalResearches = await db.query.technicalResearch.findFirst({
      where: eq(technicalResearch.id, queryExistingTechnicalResearches.data.id),
