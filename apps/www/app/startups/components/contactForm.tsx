...
import { formOpts, vcAcceleratorOptions } from "../validator";
        ...
        >
            ...
            <form.Field
              ...
              children={(field) => {
                return (
                  <FormSelect
                    className="dark"
                    placeholder="Select your VC or accelerator"
                    id={field.name}
                    label={field.name}
                    name={field.name}
                    value={field.state.value}
                    options={vcAcceleratorOptions}
                    error={field.state.meta.errors.join(",")}
          ...
          <div className="space-y-2 justify-end align-bottom">
            <form.Subscribe
              ...
              children={([canSubmit, loading, isPristine, isSubmitting, isFieldsValid]) => (
                <Button
                  size="lg"
                  className="dark w-full bg-orange-500 hover:bg-orange-400 inset-x-0 bottom-0 "
                  type="submit"
                  loading={loading}
                  disabled={!canSubmit || loading || isPristine || isSubmitting || !isFieldsValid}
                >
                  Submit
                </Button>
              )}
